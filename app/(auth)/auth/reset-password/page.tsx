...
const Page: FC<PageProps> = () => {
    ...
    const handleOnChange = (e: { target: { name: any; value: any; }; }) => {
        ...
        // Check password equality if both fields are non-empty
        if (name === 'password1' && value && formData.password2) {
            setError(value !== formData.password2);
        } else if (name === 'password2' && value && formData.password1) {
            setError(value !== formData.password1);
        }
    };
            ...
            <div className="mx-auto px-5 ">
                <div className="max-w-[414px] mx-auto">
                    <div className="flex flex-col gap-6">
                        ...
                        {error &&
                            <p className="text-pink-500 text-center">Passowrds do not match</p>
                        }
                        <form action="" method="POST" className="flex flex-col gap-6" onSubmit={handleSubmit}>
                            <div className="relative h-[21px] justify-between items-center flex mb-8">
                                <input type={`${showPassword ? 'text' : 'password'}`} placeholder="Password" defaultValue={formData.password1} onChange={handleOnChange} name="password1" className={`focus-within:outline-none w-full h-[53px] p-4 text-black rounded-xl border border-stone-300 justify-start items-center inline-flex`} />
                            ...
                            <div className="relative h-[21px] justify-between items-center flex mb-4">
                                <input type={`${showPassword ? 'text' : 'password'}`} placeholder="Confirm password" defaultValue={formData.password2} onChange={handleOnChange} name="password2" className={`focus-within:outline-none w-full h-[53px] p-4 text-black rounded-xl border border-stone-300 justify-start items-center inline-flex`} />
